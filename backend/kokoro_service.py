...
def generate_kokoro_audio(
    ...
    try:
        ...
        # Load Kokoro ONLY when endpoint is called
...
def generate_kokoro_mixed_audio(
    ...
    try:
        ...
        if _HAS_KOKORO:
            ...
            pack_a = pipeline.load_voice(
                voice_a
            )

            pack_b = pipeline.load_voice(
                voice_b
            )

            mixed_vector = (
                (1.0 - float(weight)) * pack_a
                + float(weight) * pack_b
            )
